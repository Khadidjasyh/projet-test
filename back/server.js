...
app.get('/camel-outbound-test', (req, res) => {
  const sql = `
    SELECT DISTINCT
      COALESCE(sg.pays, 'N/A') AS pays,
      COALESCE(sg.operateur, rp.operateur, 'Inconnu') AS operateur,
      src.tadig,
      src.camel_outbound AS Valeur_CAMEL_IR,
      mi.anres_value AS ValeurObservee,
      CASE
        WHEN (
          (src.camel_outbound LIKE '%CAPv1%' AND mi.anres_value LIKE '%CAMEL-1%')
          OR (src.camel_outbound LIKE '%CAPv2%' AND mi.anres_value LIKE '%CAMEL-2%')
          OR (src.camel_outbound LIKE '%CAPv3%' AND mi.anres_value LIKE '%CAMEL-3%')
        )
        THEN 'Réussi'
        ELSE 'Échoué'
      END AS Resultat,
      CASE
        ...
        WHEN src.camel_outbound IS NULL OR src.camel_outbound = '' THEN 'Valeur CAMEL Outbound manquante'
      ...
      END AS Commentaire
    FROM (
      SELECT 'IR21' AS source, tadig, e212, camel_outbound FROM ir21_data
      UNION ALL
      SELECT 'IR85' AS source, tadig, e212, camel_outbound FROM ir85_data
    ) AS src
    ...
    WHERE src.camel_outbound IS NOT NULL
    ORDER BY pays, operateur;
  `;

  connection.query(sql, (err, results) => {
    if (err) {
      console.error('Erreur API camel-outbound-test:', err);
