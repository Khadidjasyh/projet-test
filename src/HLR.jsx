...
const HlrPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const response = await fetch('http://localhost:5178/hlrr'); // Ajusté pour correspondre à l'endpoint backend
                if (!response.ok) {
    ...
    return (
        <div className="p-6">
            <div className="mb-6">
                ...
                <p className="text-gray-600">Affichage et recherche des données HLR importées</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="text-red-500 text-center p-4">
                    <p>Error: {error}</p>
                    <p className="text-sm mt-2">Vérifie que le backend fonctionne et retourne du JSON pour /hlr.</p>
                </div>
            ) : currentData.length === 0 ? (
                <p className="text-gray-500 text-center p-4">Aucune donnée HLR trouvée</p>
            ) : (
