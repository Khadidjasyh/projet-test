...
const IR85Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const response = await fetch('http://localhost:5177/ir85');  // Changez l'URL selon votre API
        if (!response.ok) {
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-4 flex items-center space-x-4">
        ...
        <button
          onClick={() => {
            // Cette fonction pourrait être implémentée plus tard pour l'import des fichiers IR85
            alert('Fonctionnalité d\'import IR85 à implémenter');
          }}
          className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center"
        >
          <span className="mr-2">Importer IR85</span>
        </button>
      </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {currentData.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 border-b">{new Date(row.date_ajout).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
