...
const SituationGlobale = () => {
  ...
  const handleImportClick = (operatorId) => {
    console.log('handleImportClick appelé avec ID:', operatorId);
    setSelectedOperatorId(operatorId);
    setImportModalOpen(true);
    console.log('importModalOpen mis à:', true);
  };
  ...
  const handleImportSubmit = async () => {
    console.log('handleImportSubmit appelé');
    if (!file) {
      toast.error('Veuillez sélectionner un fichier IR21');
      return;
    }

    console.log('Préparation du FormData pour l\'upload');
    const formData = new FormData();
    formData.append('file', file);
    formData.append('operatorId', selectedOperatorId);
    console.log('FormData prêt, fichier:', file.name, 'operatorId:', selectedOperatorId);
    ...
    try {
      ...
      toast.success(`Fichier IR21 ${fileName} importé avec succès`);
      setImportModalOpen(false);
      setFile(null);
      setFileName('');
      
      if (result.updatedOperator) {
        setData(data.map(item => 
          item.id === selectedOperatorId ? { ...item, ...result.updatedOperator } : item
        ));
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      ...
      {/* Modal d'importation IR21 */}
      {importModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-xl font-bold mb-4">Importer un fichier IR21</h2>
            
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Sélectionner un fichier IR21
              </label>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center cursor-pointer hover:border-blue-500 transition-colors">
                    {file ? (
                      ...
                      <div className="text-gray-500">
                        Cliquez pour sélectionner un fichier IR21
                      </div>
                    )}
                    <input
                      type="file"
              ...
              <p className="text-xs text-gray-500">
                Formats acceptés: .csv, .pdf, .xml, .txt
              </p>
            </div>
