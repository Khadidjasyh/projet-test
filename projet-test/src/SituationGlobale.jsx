...
const SituationGlobale = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch('/situation-globale');
  ...
  const handleSaveAll = async () => {
    try {
      ...
      for (const [id, data] of Object.entries(modifiedItems)) {
        const response = await fetch(`/situation-globale/${id}`, {
          method: 'PUT',
          headers: {
  ...
  const handleImportSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post('/import-ir21', formData, {
        headers: {
      ...
      if (response.data.success) {
        toast.success(response.data.message || 'Import réussi');
        
        // Mettre à jour les données locales
        const updatedData = data.map(item => {
          if (item.id === selectedOperatorId) {
            return {
              ...item,
              gsm: response.data.data.gsm,
              gprs: response.data.data.gprs,
              troisg: response.data.data.troisg,
              lte: response.data.data.lte,
              camel: response.data.data.camel,
              mcc: response.data.data.mcc,
              mnc: response.data.data.mnc,
              epcrealm: response.data.data.epcrealm,
              imsi: response.data.data.imsi
            };
          }
          return item;
        });
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="flex justify-end space-x-3">
              ...
              >
                {file ? 'Importer' : 'Sélectionnez un fichier'}
              </button>
            </div>
          </div>
        </div>
