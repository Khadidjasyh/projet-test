...
import { BsFillShieldLockFill, BsExclamationTriangleFill, BsCheckCircleFill, BsGearFill, BsBell, BsGear, BsSpeedometer2, BsFileEarmarkText, BsPeople, BsArrowLeftRight, BsExclamationTriangle, BsFileEarmarkBarGraph, BsQuestionCircle, BsBoxArrowRight, BsTable, BsCheckCircle, BsExclamationCircle, BsXCircle } from 'react-icons/bs';
...
const Dashboard = () => {
   const [selectedType, setSelectedType] = useState('gsm');
  const [mapData, setMapData] = useState({});
  const [geoJson, setGeoJson] = useState(null);
  const [lastAuditReport, setLastAuditReport] = useState(null);

  const getColor = (status) => {
    switch (status) {
      case 'ok': return '#2ecc71';
      case 'issue': return '#e74c3c';
      case 'no_agreement': return '#bdc3c7';
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/api/map/${selectedType}`)
      .then(res => res.json())
      .then(setMapData)
  ...
  useEffect(() => {
    fetch('/api/audit-report/last')
      .then(res => res.json())
      .then(setLastAuditReport)
      .catch(err => console.error('Error fetching last audit report:', err));
  }, []);
  ...
  const [recentTestAudits, setRecentTestAudits] = useState([]);
  ...
  useEffect(() => {
    fetch('http://localhost:5000/dashboard/critical-alerts')
      .then(res => res.json())
      .then(setCriticalAlerts)
      .catch(err => console.error('Error fetching critical alerts:', err));
  }, []);

  useEffect(() => {
    fetch('http://localhost:5000/dashboard/recent-audits')
      .then(res => res.json())
      .then(setRecentAudits)
      .catch(err => console.error('Error fetching recent audits:', err));
  }, []);

  useEffect(() => {
    fetch('http://localhost:5000/dashboard/recent-test-audits')
      .then(res => res.json())
      .then(setRecentTestAudits)
      .catch(err => console.error('Error fetching recent test audits:', err));
  }, []);
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-500 mb-1">Partenaire</p>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-500 mb-1">Rapports</p>
  ...
  <div className="flex gap-4">
    <button
      onClick={() => setSelectedType('gsm')}
      className={`px-4 py-2 rounded ${selectedType === 'gsm' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
    >
      GSM
    </button>
    <button
      onClick={() => setSelectedType('camel')}
      className={`px-4 py-2 rounded ${selectedType === 'camel' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
    >
      CAMEL
    </button>
  </div>
  ...
  <div className="ml-auto flex items-center gap-4">
    <span className="flex items-center gap-1 text-sm">
      <span className="w-4 h-4 bg-green-500 rounded-full"></span> Bilateral
    </span>
    <span className="flex items-center gap-1 text-sm">
      <span className="w-4 h-4 bg-red-500 rounded-full"></span> Issue
    </span>
    <span className="flex items-center gap-1 text-sm">
      <span className="w-4 h-4 bg-gray-400 rounded-full"></span> No Agreement
    </span>
    <span className="flex items-center gap-1 text-sm">
      <span className="w-4 h-4 border border-black rounded-full bg-white"></span> No Info
    </span>
  </div>
</div>

                    <div className="rounded-lg overflow-hidden border" style={{ minHeight: 350, height: 400 }}>
                        ...
                        {geoJson && (
                          <GeoJSON
                            ...
                            onEachFeature={(feature, layer) => {
                              ...
                              layer.bindPopup(`${countryName} : ${status || 'Aucune info'}`);
                            }}
                          />
                        )}
                      </MapContainer>
                ...
                {/* Right column: Alertes Critiques */}
                <div className="bg-white p-6 rounded-lg shadow-md h-fit min-h-[570px] flex flex-col justify-between">
               ...
               <Card>
                <div className="flex items-center justify-between">
                   {/* Audits récents card */}
                  <div
      className="bg-white p-6 rounded-lg shadow-md mt-6 flex flex-col items-start"
      style={{ width: "1000px", height: "255px" }}
    >
      <h2 className="text-lg font-semibold mb-4">Audit Rapport </h2>
        ...
        <table className="min-w-full table-auto">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="px-4 py-2 text-left">Totale erreur</th>
            </tr>
          </thead>
          <tbody>
            {recentAudits.map((audit) => (
              <tr key={audit.audit_id} className="border-t">
                ...
                <td className="px-4 py-2">{audit.total_issues}</td>
              </tr>
            ))}
      ...
      style={{ width: "1000px", height: "255px" }}
    >
      <h2 className="text-lg font-semibold mb-4">Tests Audits</h2>
      <div className="overflow-x-auto w-full">
        <table className="min-w-full table-auto">
          <thead>
            <tr className="bg-gray-100">
              <th className="px-4 py-2 text-left">Id</th>
              <th className="px-4 py-2 text-left">Nom</th> {/* changé */}
              <th className="px-4 py-2 text-left">Description</th>
              <th className="px-4 py-2 text-left">Statut</th>
            </tr>
          </thead>
          <tbody>
            {recentTestAudits.map((audit) => (
              <tr key={audit.audit_id} className="border-t">
                <td className="px-4 py-2">{audit.audit_id}</td>
                <td className="px-4 py-2">{audit.Name}</td>
                <td className="px-4 py-2">{audit.description}</td>
                <td className={`px-4 py-2 font-semibold ${getStatusColor(audit.Status)}`}>
                  {audit.Status}
                </td>
                <td className="px-4 py-2">{audit.Statu}</td>
              </tr>
            ))}
            {recentTestAudits.length === 0 && (
              <tr>
